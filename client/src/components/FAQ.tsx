...
const FAQ = (props: React.PropsWithoutRef<FAQProps>) => {
    ...
    return (
        <div className={`${props.isOpen ? "bg-purple-100/60" : ""} rounded`}>
            <button onClick={openFaqHandler} className="grid grid-cols-6 justify-between items-center w-full text-left p-3 font-medium">
            ...
            {props.isOpen ?
                <p className="px-3 pb-3 text-slate-600 text-[.93rem] leading-relaxed">{props.answer}</p>
                :
                null
            }
        </div>
