...
const Input = (props: React.PropsWithoutRef<InputWithLabelProps | InputWithoutLabelProps>) => {
    return (
        <div className="relative pb-6 flex flex-col gap-1">
            ...
            <input onChange={props.onChange} value={props.value} defaultValue={props.defaultValue} name={props.name} type={props.type} id={props.id} placeholder={props.placeHolder} className={`border-2 rounded bg-white outline-slate-400 px-2 py-1 ${props.isError ? "border-red-300 outline-red-300" : "border-slate-300"}`} />
            {props.isError ?
