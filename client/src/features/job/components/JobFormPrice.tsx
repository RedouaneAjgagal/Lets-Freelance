            ...
            <div key={price} className={`${props.priceType === "fixed" ? "w-full" : ""} flex flex-col gap-1`}>
                ...
                <div className="relative">
                    <div className="flex items-center gap-1">
                        <input id={`job_price_${price}`} name={`job_price_${price}`} type="number" className={`${props.priceType === "hourly" ? "max-w-[7rem]" : "max-w-full"} border-2 px-2 py-1 rounded flex items-center gap-1 justify-between w-full outline-slate-400 pl-7 ${props.isError ? "border-red-300" : "border-slate-300"}`} min={1} defaultValue={price === "budget" ? props.defaultValue.max : props.defaultValue[price]} />
