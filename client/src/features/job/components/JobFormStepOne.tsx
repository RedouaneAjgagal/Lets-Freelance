...
const JobFormStepOne = (props: React.PropsWithoutRef<JobFormStepOneProps>) => {
    ...
    const experienceLevelInputs = experienceLevelOptions.map(experienceLevel => {
        ...
        return (
            <div key={id} className="flex gap-2">
                <input className="accent-purple-600" type="radio" name="job_experienceLevel" id={id} value={experienceLevel} defaultChecked={experienceLevel === "entryLevel"} />
    ...
    return (
        <section className={`${props.isCurrentStep ? "flex flex-col not-sr-only" : "hidden sr-only"}`}>
            ...
            <div className="relative pb-6 flex flex-col gap-1">
                ...
                <SelectOptions options={options} onSelect={onSelectOption} selectTitle={category} isError={props.errors.category} />
