            ...
            <>
                ...
                <section className='fixed w-[90%] bg-white left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 p-4 rounded shadow-lg flex flex-col gap-5 max-w-[45rem] sm:p-6'>
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <select name="amount_to_buy" className="w-full border border-slate-600 rounded py-2 px-1" onChange={selectAmountHandler}>
                            {options}
                        </select>
                    </div>
                    <div className="flex flex-col gap-1">
                    ...
                    <div className='flex justify-end gap-2 sm:gap-4'>
                        <button className="text-purple-800 font-medium px-2 sm:px-4" onClick={props.onClose}>Cancel</button>
                        <PrimaryButton isLoading={buyConnectsMutation.isLoading} disabled={buyConnectsMutation.isLoading} fullWith={false} justifyConent='center' style='solid' type='button' x='lg' y='lg' onClick={buyConnectsHandler}>Buy Connects</PrimaryButton>
                    </div>
                </section>
            </>
            ,
