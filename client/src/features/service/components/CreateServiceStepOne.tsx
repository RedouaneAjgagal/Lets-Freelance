        ...
        <CreateServiceWrapper title="General">
            <div className="grid grid-cols-1 md:grid-cols-2 md:gap-4">
                <InputContainer label="Title" error={title.error.msg} id="title">
                    <input name="title" onChange={setTitleHandler} type="text" id="title" value={title.value} className={`border-2 rounded outline-slate-400 px-2 h-10 ${title.error.msg ? "border-red-300 outline-red-300" : "border-slate-300"}`} />
                </InputContainer>
                <InputContainer label="Category" error={category.error.msg} id="category" >
                    <select onChange={setCategoryHandler} name="Category" id="category" className="border-2 rounded border-slate-300 outline-slate-400 px-1 h-10" value={category.value}>
