...
const TierContainer = (props: React.PropsWithoutRef<TierContainerProps>) => {
    ...
    return (
        <div className="flex flex-col bg-white">
            <button onClick={openTierHandler} className={`${props.isTierOpen ? "rounded-t" : "rounded"} ${props.isError && !props.isTierOpen ? "border-red-300" : "border-slate-300"} flex items-center justify-between w-full border-2 px-2 py-3 font-semibold text-purple-700 text-lg tracking-wide uppercase md:px-3`}>
                {props.tierName}
                <BsArrowDown className={`${props.isTierOpen ? "-rotate-180" : "-rotate-0"} transition-all duration-300`} />
            </button>
            {props.isTierOpen ?
                <div className="border-2 border-t-0 rounded-b border-slate-300 p-3 md:p-6">
                    ...
                    <div className="relative pb-6 flex flex-col gap-2 md:pb-0">
                        ...
                        {props.tierName !== "starter" && !tier[props.tierName].includedIn.value.length && tier[props.tierName === "standard" ? "starter" : "standard"].includedIn.value.length ?
                            <button onClick={copyIncludedInHandler} className="self-end text-sm p-1 underline text-purple-500 md:self-start">Copy the previous</button>
                            : null
                        }
                        <button onClick={addMoreInludedInHandler} className="bg-purple-200/30 border-purple-300 p-2 rounded border flex justify-center items-center text-slate-900 md:self-start md:w-56">
                            <BsPlus size={24} />
                        </button>
                    </div>
                </div>
